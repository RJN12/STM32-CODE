...
#define SAMPLES 30              // ADD THIS
uint32_t adc_samples[SAMPLES];  // ADD THIS
...
void lcd_send_cmd(char cmd)
{
    ...
    HAL_I2C_Master_Transmit(&hi2c2, (ADDRESS_LCD << 1), data_t, 4, 100);
}
...
int main(void)
{
  ...
  RTC_TimeTypeDef current_date;
    ...
    char msg[] = "TDS Sensor Started for YT video\r\n";
      ...
      /* USER CODE END WHILE */
  	  if(Check_Time(2))
  	  {
  	      ...
  	      int len = snprintf((char*)uart_buf, sizeof(uart_buf),
  	                        "ADC: %lu | V: %d.%03dV | TDS: %d ppm\r\n",
  	                        adc_value,
  	                        volt_mv/1000, volt_mv%1000,
  	                        tds_int);
  	    ...
  	    lcd_goto_xy(1, 0);
  	    char lcd_buf[16];
  	    snprintf(lcd_buf, 16, "TDS: %d ppm", tds_int);
  	    lcd_send_string(lcd_buf);

  	    lcd_goto_xy(2, 0);
  	    snprintf(lcd_buf, 16, "V: %d.%02dV", volt_mv/1000, (volt_mv%1000)/10);
  	    lcd_send_string(lcd_buf);
